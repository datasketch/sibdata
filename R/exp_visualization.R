# exp_visualization.R
# Visualization Module for SIB Data App (modular version)
# Uses ONLY centralized reactive values - no internal reactives

exp_visualization_ui <- function(id) {
  ns <- NS(id)
  tagList(
    # Type selector and chart selector row
    div(style = "display: flex; justify-content: space-between;",
        ...
        div(style = "flex: 1; text-align: right;",
            radioButtons(ns("chart_type"), "Visualización",
...
exp_visualization_server <- function(id, r, con) {
  moduleServer(id, function(input, output, session) {
    ...
    output$viz_output <- renderUI({
      ...
      if (r$chart_type == "map") {
        leaflet::leafletOutput(ns("map_viz"), height = 450)
      } else {
